...
test("access-user-profile-and-update-profile", async ({ page }) => {
  ...
  const imageLinkInput = page.getByRole("textbox", {
    name: "Text field for the image link",
  });
  await expect(imageLinkInput).toBeVisible();
  await imageLinkInput.fill("https://example.com/profile-picture.png");
